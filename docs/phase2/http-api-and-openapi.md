...
机器契约为 `contracts/phase2/http-api.json`，错误体为 `http-error.v1`，OpenAPI 文档由 `build_openapi_document()` 生成并由 `/openapi.json` 返回。实现位于 `src/brand_os/http_api.py`。F2.9 已把请求关联、健康指标、追踪和共享限流故障语义接到同一入口。
...
当前覆盖 400、401、403、404、409、422、429 和 503。每个响应带 `X-Request-ID`、`X-Correlation-ID` 和 `traceparent`；未提供或格式不安全的请求 ID 不会被原样回显。默认限流器是进程内固定窗口实现，仅适合开发或单进程部署；多副本服务器必须注入 PostgreSQL v11 共享实现。共享存储故障返回 503 `RATE_LIMIT_STORE_UNAVAILABLE`，不回退到本地计数。

## 不在本任务内的内容

- 远程 MCP OAuth、MCP 命令身份和工具白名单：F3.6。
- OpenWork 登录界面、钥匙串和服务器项目选择：F3.2。
- 日志、指标、追踪、告警和共享限流存储：已由 F2.9 完成，详见[可观测性、健康和告警](observability-and-alerting.md)。
- SQLite 到 PostgreSQL/S3 的一次性切换：F3.1。
- Dify、Zvec、Open Notebook、Nubase 和 FlowLong 分别按 F3.14-F3.18 接入或评估；BISHENG 仍是 Phase 4 真实试点后的独立候选，不因本 API 发布而提前启动。
