...
BISHENG 仍是当前 49 项完成后的候选工作流实现，不是 F2.9 的依赖。将来若 Fox 在 Phase 4 之后单独批准 BS0-BS3 POC，BISHENG 适配器必须复用同一套请求关联、脱敏、Outbox 观测和告警契约；BISHENG 的运行状态只能作为派生工作流状态回到 OpenWork，不能成为正式状态源或人工批准入口。
