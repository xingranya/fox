...
- 当前 `postgresql-authority.v6` 继续引用 `oidc-identity.v1`；OIDC 表由 v8 引入，v9 增加项目授权，v10 增加审计和派生投递，v11 增加共享限流窗口。
- 当前 `server-boundary.v4` 继续把 OIDC 和身份会话存储列为必需组件，并将 Outbox Worker 明确为不能推进正式状态的可选后台组件。
