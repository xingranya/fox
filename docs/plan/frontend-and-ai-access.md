...
本地 MCP 使用 stdio，不要求远程 OAuth。F1.8 已实现：
...
会议、全文搜索和行动项工具尚未实现，待对应应用端口稳定后再加入。MCP 工具不包含批准、驳回、直接 SQL、成员管理、原件硬删除或工作模式强制切换。项目在 MCP 启动时固定，工具参数不能切换项目；客户端不会获得 `/Users/fox/work` 的任意文件读取能力。
...
- Codex 与 Claude 的适配配置只包含本地 stdio MCP 命令。模型登录和凭据由各自运行时管理，Brand Project OS 不读取提供商密钥。
