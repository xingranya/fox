...
> 计划位置：Phase 4 真实试点后的候选池，不计入当前 56 项完成度
...
这份 SPEC 只把 BISHENG 加入后续候选池，不批准部署或产品代码。进入 BS0 的最早条件是：Phase 4 已形成真实团队试点结论，Fox 明确批准启动候选 POC，并同意用同一输入对比 BISHENG、Dify 或直接 Worker。届时应执行一次正式 rescope，把 BS0-BS3 作为独立任务段纳入活动进度；在此之前不得把它写成当前 F3.3-F4.10 的依赖或完成条件。
