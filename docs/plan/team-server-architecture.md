...
> 当前进度：F2.1-F2.2 已通过，当前执行 F2.3 S3 兼容原件版本、哈希和准入状态机<br>
> 当前生效决策：[ADR-0005：单一客户端与服务器权威服务](../adr/0005-single-client-server-authority.md)

F2.1 已完成配置优先级、秘密注入、服务器组件职责和健康报告基线。F2.2 已完成 PostgreSQL v1-v6 迁移与权威存储适配器，并在一次性临时 PostgreSQL 17 集群完成集成验证。当前仍不启动常驻服务，不执行 S3/OIDC 初始化，也不迁移鸿日正式数据。
...
- F2.1 已冻结服务器配置、职责和健康基线，F2.2 已完成 PostgreSQL 权威存储适配；当前从 F2.3 开始，依次完成对象存储、身份权限、一致性、恢复、联网客户端和团队试点。
