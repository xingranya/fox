...
以下档位用于 Phase 2-4。Fox 已批准小团队托管部署作为第一批内部试点档位；Phase 4 用真实负载和恢复演练决定是否维持该档位或升级高可用：
...
| 小团队生产 | 应用节点 + 托管 PostgreSQL + 托管对象存储 + 独立备份域 | 签名稳定版桌面；按需托管 Worker | 第一批内部成员 | 已批准目标 5 分钟 / 60 分钟 | 当前批准档位；Phase 4 实测 |
...
Fox 已批准上表作为小团队托管部署的内部目标。所有目标仍必须以 Phase 4 的压测、监控和独立恢复演练证明，不能只依据云服务商或上游项目宣传。AI 生成时延按运行时和模型单独统计，不混入核心 API SLO。
