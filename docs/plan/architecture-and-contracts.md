...
> Phase 1：本地 SQLite 纵切，已完成；F2.1-F2.8 已通过，当前 F2.9；Phase 2-4：服务器权威、联网客户端和团队试点
...
健康语义也已冻结：`live` 只证明进程能响应；`ready` 检查必需配置、PostgreSQL、Schema、对象存储和 OIDC。OpenWork Runtime、Dify、Zvec、Open Notebook、Nubase 和 FlowLong 属于可选依赖，故障只记录为降级，不能让核心 API 失去就绪状态。F2.8 已发布 `/livez`、`/readyz`，真实指标、追踪和告警仍留给 F2.9。
...
机器契约已随 F2.7 升级为 `postgresql-authority.v5`：v1-v6 是领域语义，v7 增加对象准入元数据，v8 增加员工身份与会话，v9 增加项目授权和 RLS，v10 增加审计与 Outbox/Inbox。完整实现和测试边界见 [F2.2 PostgreSQL 权威事件、审批和投影](../phase2/postgresql-authority-store.md) 与 [F2.7 审计、Outbox/Inbox 和后台任务](../phase2/audit-outbox-inbox.md)。F2.8 已发布 HTTP API，但当前仍没有迁移鸿日数据、没有双写；F3.1 正式切换前，鸿日仍以 Phase 1 SQLite 为权威。
...
F2.7 发布 `audit-outbox.v1` 和 PostgreSQL v10。领域事件、人工审批、正式投影、审计与 Outbox 在同一事务中提交；提交不等待消费者。Outbox 按消费者拆分投递行，`FOR UPDATE SKIP LOCKED` 加租约实现至少一次投递，同一聚合按版本阻止乱序。Inbox 以 `(consumer_name, event_id)` 去重，失败按重试策略进入可审计死信，重放不改变项目版本。独立 Worker 角色没有正式表写权限。详见 [F2.7 审计、Outbox/Inbox 和后台任务](../phase2/audit-outbox-inbox.md)。
...
- 客户端始终通过应用 API 传输证据，不获得 PostgreSQL、S3 或任意 SQL 能力。默认限流器只适合单进程，团队多副本共享实现和指标告警由 F2.9 补齐。
...
Phase 1 本地实现为 SQLite；服务器 PostgreSQL 当前使用 v1-v10，其中 v1-v6 承载领域语义，v7 承载对象元数据，v8 承载员工身份与会话，v9 承载项目授权与 RLS，v10 承载审计和派生投递。F3.1 切换前两者不双写，鸿日仍以 SQLite 为正式权威。一次 `execute` 必须在一个事务内：
...
| `postgresql-authority.v5` | PostgreSQL v1-v10 权威、对象、身份、授权和派生边界 | v1-v6 领域事务不变；v7 对象准入；v8 员工身份；v9 项目授权与 RLS；v10 审计/Outbox/Inbox |
...
|:---|:---|:---|
| SQLite `CanonicalStorePort` | PostgreSQL v1-v10、RLS/Outbox 和 HTTP API 已完成；正式迁移待 F3.1 | 稳定 ID、事件顺序、人工批准、版本冲突和派生投递语义 |
