...
> Phase 1：本地 SQLite 纵切，已完成；F2.1-F2.9 已通过，当前 F2.10；Phase 2-4：服务器权威、联网客户端和团队试点
...
健康语义也已冻结：`live` 只证明进程能响应；`ready` 检查必需配置、PostgreSQL、Schema、对象存储和 OIDC。OpenWork Runtime、Dify、Zvec、Open Notebook、Nubase 和 FlowLong 属于可选依赖，故障只记录为降级，不能让核心 API 失去就绪状态。F2.8 发布 `/livez`、`/readyz`，F2.9 已补齐观测契约、请求追踪、Outbox 水位和共享限流故障语义。

机器契约当前为 `server-boundary.v4`、`service-config.v2`、`service-health.v1`。服务器边界明确只有应用服务可以推进正式状态；OIDC 只认证员工，项目授权必须先于存储访问；Employee API、MCP Gateway、OpenWork Runtime、Outbox Worker 和工作流不能把服务身份变成人工审批，存储适配器不得被客户端或 Agent 直连。
...
机器契约已随 F2.9 升级为 `postgresql-authority.v6`：v1-v6 是领域语义，v7 增加对象准入元数据，v8 增加员工身份与会话，v9 增加项目授权和 RLS，v10 增加审计与 Outbox/Inbox，v11 增加共享限流窗口。完整实现和测试边界见 [F2.2 PostgreSQL 权威事件、审批和投影](../phase2/postgresql-authority-store.md)、[F2.7 审计、Outbox/Inbox 和后台任务](../phase2/audit-outbox-inbox.md) 与 [F2.9 可观测性、健康和告警](../phase2/observability-and-alerting.md)。F2.8 已发布 HTTP API，但当前仍没有迁移鸿日数据、没有双写；F3.1 正式切换前，鸿日仍以 Phase 1 SQLite 为权威。
...
F2.6 已实现 `write-consistency.v1` 和 `write-conflict.v1`。正式写调用统一返回 `COMMITTED`、`REPLAYED` 或 `CONFLICT`；同一幂等键更换请求摘要、版本过期和资源状态已变化使用不同冲突代码。F2.8 已将冲突报告稳定映射为 HTTP 409，未改变领域语义。
...
F2.7 发布 `audit-outbox.v1` 和 PostgreSQL v10。领域事件、人工审批、正式投影、审计与 Outbox 在同一事务中提交；提交不等待消费者。Outbox 按消费者拆分投递行，`FOR UPDATE SKIP LOCKED` 加租约实现至少一次投递，同一聚合按版本阻止乱序。Inbox 以 `(consumer_name, event_id)` 去重，失败按重试策略进入可审计死信，重放不改变项目版本。独立 Worker 角色没有正式表写权限。F2.9 在 v11 增加共享限流窗口，并把 Outbox 水位接入低基数观测。详见 [F2.7 审计、Outbox/Inbox 和后台任务](../phase2/audit-outbox-inbox.md) 与 [F2.9 可观测性、健康和告警](../phase2/observability-and-alerting.md)。

### F2.8 版本化 HTTP API 与 OpenAPI（已完成）

F2.8 发布 `http-api.v1`、`http-error.v1` 和 OpenAPI 3.1。实现位于 `src/brand_os/http_api.py`，应用通过依赖注入复用身份、项目授权、证据准入、权威存储和 `WriteConsistencyService`；领域核心不依赖 Starlette。

- `/api/v1/employee/**` 只接受 OIDC 交互式员工会话，包含登录回调、状态、证据、Proposal 和人工评审。
- `/api/v1/agent/**` 只接受不透明服务凭据，允许读取、回源、Task Packet 和创建 Proposal；没有人工评审路由。
- 正式写请求强制 `Idempotency-Key` 与 `If-Match`，`COMMITTED`/`REPLAYED`/`CONFLICT` 分别映射 201/200/409。
- 列表使用绑定项目状态版本的 HMAC 游标；最大页大小 100。错误体统一带 `request_id`，覆盖 400/401/403/404/409/422/429/503。
- 客户端始终通过应用 API 传输证据，不获得 PostgreSQL、S3 或任意 SQL 能力。默认限流器只适合单进程，多副本使用 PostgreSQL v11 共享实现；后端故障必须 fail closed 并返回稳定 503。
...
Phase 1 本地实现为 SQLite；服务器 PostgreSQL 当前使用 v1-v11，其中 v1-v6 承载领域语义，v7 承载对象元数据，v8 承载员工身份与会话，v9 承载项目授权与 RLS，v10 承载审计和派生投递，v11 承载共享限流窗口。F3.1 切换前两者不双写，鸿日仍以 SQLite 为正式权威。一次 `execute` 必须在一个事务内：
...
| `server-boundary.v4` | 服务器组件职责 | 只有应用服务推进正式状态；OIDC 只认证员工；项目授权先于存储；OpenWork Runtime 和 Outbox Worker 不是业务服务 |
...
| `http-api.v1` / `http-error.v1` | 版本化 HTTP 入口 | Employee/Agent 分路、分页、错误映射、兼容窗口和限流边界 |
| `postgresql-authority.v6` | PostgreSQL v1-v11 权威、对象、身份、授权、派生和共享限流边界 | v1-v6 领域事务不变；v7 对象准入；v8 员工身份；v9 项目授权与 RLS；v10 审计/Outbox/Inbox；v11 共享限流 |
| `audit-outbox.v1` | 审计、至少一次派生投递和 Inbox 去重 | 消费者可重试、可死信、可重放，但不能写正式业务表 |
...
| `project-authorization.v1` | 项目角色、动作、服务身份和保密上限 | 应用先判权、服务无批准权、事务级上下文和强制 RLS |
...
|:---|:---|:---|
| SQLite `CanonicalStorePort` | PostgreSQL v1-v11、RLS/Outbox、HTTP API 和观测边界已完成；正式迁移待 F3.1 | 稳定 ID、事件顺序、人工批准、版本冲突和派生投递语义 |
| 本地证据区 | S3 兼容内容寻址对象准入已完成；正式迁移待 F3.1 | SHA-256、来源版本、原文定位和不可变性 |
| 本地 OS 用户 | OIDC 身份会话、项目角色、服务 Scope 和 RLS 已完成 | 人工与 AI 身份分离、AI 禁止批准 |
