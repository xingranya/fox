...
- 最新稳定发布：`v0.17.20`（2026-07-10）。
- 源码结构观察快照：`dev@316b996`（2026-07-12）。
- `dev` 只用于理解近期结构，不是固定采用基线。
...
- `.fox/runtime/`、OpenWork 数据和 Agent 会话均可删除；`.fox/project.db` 与证据区不由 OpenWork 迁移或清理逻辑管理。
...
| 工作包 | 内容 | 产出 | 继续门 |
|:---|:---|:---|:---|
| OW-L0 | 固定评估 SHA、许可证/`ee`/网络出口扫描、构建最小桌面 | 评估记录与可启动本地壳 | 无 `ee`/Den/上游云硬依赖 |
...
OW-L0 通过。不能构建、必须连接上游云、必须使用 `ee/**` 或不能关闭未登记网络出口时立即停止。
