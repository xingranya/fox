...
> 状态：Phase 1 和 Phase 2 已通过，F3.1 已完成，当前执行 F3.2<br>
> 当前目标：保留公司定制 OpenWork 这一唯一员工客户端，完成权威数据一次性迁移后，接入 OIDC、项目选择和服务器联网闭环<br>
...
本计划同时约束已完成的 Phase 1 本地纵切和 Phase 3 联网形态。F1.10 已通过，现在按[任务分解](task-breakdown.md)建设服务器权威层，再把同一个客户端切到版本化 API。服务器不是第二个员工软件。
...
OW-L0 已有条件通过，F1.9 已完成离线、安全、品牌和单安装包收口。当前 fork 默认关闭 PostHog、Den/Cloud、模型目录和上游更新，已更换内部工作名/AppID/协议/数据目录并收紧 Electron 网络权限。F1.10 只读接入授权范围内的真实鸿日资料，来源和权威数据库哈希在验收前后保持一致。
...
OpenWork 已固定为员工客户端基础，Fox 的 Phase 1 真实桌面体验已通过。当前 `.app` 尚未签名和公证，仍不向员工分发；内部更新、灰度和回滚由 F4.8 过门，不切换到第二套员工客户端。
...
实施顺序和阶段门见[任务分解](task-breakdown.md)与[里程碑](milestones.md)。Phase 2 从 F2.1 开始，不把后续服务器能力补写成 Phase 1 的通过条件。
