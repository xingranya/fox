...
任何阶段门未通过时，下游任务保持未开始。F1.10 和 F2.10 已通过，Phase 2 已关闭，F3.1 已完成，当前进入 F3.2 OpenWork 联网闭环。BISHENG 候选池不属于当前阶段依赖。
...
- F2.10 已通过；F3.1 对账和切换门未通过前，SQLite 继续保持权威且不得与 PostgreSQL 双写。
