...
- 控制面补充：[ADR-0007](0007-adopt-openwork-den-control-plane.md)
...
2026-07-23，Fox 通过 [ADR-0006](0006-foxwork-name-and-chinese-ui.md) 将客户端发行名固定为 FoxWork，并要求员工界面只使用简体中文；2026-07-24 又将范围扩展到 Den 后台管理员网页。这不改变本 ADR 的单一客户端、服务器权威和权限边界。
...
3. 服务器分为两个明确边界：Den 负责单组织自助注册、账号、组织、团队、远程工作区/Worker、MCP、Skills、共享模型和桌面策略；Brand Project OS Service 负责版本化业务 API、权威事件与审批、当前投影、证据元数据、MCP Gateway、多媒体处理、审计和工作流适配。
...
6. MCP 是 Agent 访问协议，不是业务核心。FoxWork 使用 Brand OS 业务 API 完成员工操作；Brand OS MCP 登记到 Den 并按成员/团队下发，Codex、Claude、Dify 和其他 Agent 通过受控 MCP/Skills 调用同一应用服务。
...
9. 原始证据按 SHA-256、来源版本和对象 VersionId 管理。视频、录音、图片、PPT、Office、PDF 的 OCR、转写、摘要、Zvec、Open Notebook、Nubase、Dify、FlowLong 和模型会话结果均为派生数据，不得成为正式状态源。
...
12. 第一批内部试点采用小团队托管部署：单应用节点、托管 PostgreSQL、开启版本控制的对象存储和独立备份域。内部目标为核心 API 月可用性不低于 99.5%、PostgreSQL RPO 不高于 5 分钟、核心服务 RTO 不高于 60 分钟。该决策确定目标，不代表生产验证已经通过；Phase 4 必须用真实部署复核。
...
flowchart LR
    ...
    DESKTOP --> DEN["OpenWork Den\n账号与 AI 控制面"]
    DEN --> DENDB[("MySQL\n账号 / 组织 / 能力授权")]
    DEN --> REMOTE["远程工作区 / Den Worker"]
    DEN -->|"第一方 OAuth/OIDC"| API["Brand Project OS API"]

    AGENT["Codex / Claude / 其他 Agent"] --> DEN
    DEN --> MCP["Brand OS MCP Gateway"]
    DIFY["Dify / 工作流"] --> MCP
    MCP --> APP["版本化应用服务"]
    API --> APP
