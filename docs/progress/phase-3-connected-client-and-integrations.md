...
- [ ] **F3.2：在 FoxWork 接入 OIDC、项目选择和服务器连接**
  - P0 / L / Lane B；依赖 F3.1；S.U.P.E.R：S、P、E。
  - 验收：一个客户端完成登录与项目进入；撤权、证书错误和离线状态可理解；新增员工界面只显示自然、可执行的简体中文。
...
- [ ] **F3.13：完成 FoxWork 单安装包、服务器、MCP/Skills 和工作流端到端验收**
  - P0 / XL / Gate；依赖 F3.1-F3.12；S.U.P.E.R：全部。
  - 验收：核心旅程不依赖可选组件；发行名、图标、AppID、协议和更新源均由公司控制；员工界面只提供简体中文，没有语言切换、英文回退或未解析文案键；每个外部组件有采用/试用/拒绝结论和 NoOp 回退。
...
Phase 2 已于 2026-07-23 关闭。F3.1 已用临时 SQLite、PostgreSQL 和 Moto S3 验证迁移、对账、写入冻结与回滚；真实 F1.10 基线只使用副本做只读核验，未写入真实资料。下一项是 F3.2 FoxWork 的 OIDC、项目选择和服务器连接。Fox 已把客户端发行名固定为 FoxWork，并要求员工界面只使用简体中文；任务数和实施顺序不变。
