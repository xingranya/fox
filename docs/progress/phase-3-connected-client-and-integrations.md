...
**状态**：进行中，0/13，当前任务 F3.1
**任务数**：13
**自适应阈值**：标注 3 / 重计划 6 / 重定范围 8

## 任务

- [ ] **F3.1：实现 SQLite 到 PostgreSQL/S3 的一次性迁移和切换**
  - P0 / XL / Lane A；依赖 F2.10；S.U.P.E.R：U、P、E、R。
  - 验收：ID、事件、审批、投影、来源和哈希对账；切换后本地库只读，无长期双写。
...
Phase 2 已于 2026-07-23 关闭。F3.1 先用临时 SQLite、PostgreSQL 和 Moto S3 验证迁移、对账、写入冻结与回滚；真实 F1.10 基线只使用副本做只读核验，迁移门通过前不写真实资料。
