...
| 1 | 鸿日本地单用户原型 | 10 | 2 | 20% | 本地资料、SQLite 状态、会议增量、证据、Task Packet、多模型与简单界面闭环 |
| 2 | 鸿日真实工作连续验证 | 8 | 0 | 0% | 真实增量、策略/执行、多模型、匿名评审、错误修订和价值 Go/No-Go |
| 3 | 团队服务器化决策门 | 7 | 0 | 0% | 团队需求、候选架构/组件证据和独立后续 SPEC 决策 |
| **合计** |  | **32** | **9** | **28%** | Phase 0-2 是当前价值验证；Phase 3 条件启动 |

## 阶段清单

- [x] Phase 0：边界、协议与黄金测试先行（7/7）- [详情](phase-0-boundary-and-bench.md)
- [ ] Phase 1：鸿日本地单用户原型（2/10）- [详情](phase-1-hongri-local-prototype.md)
...
1. 完成 F1.3：把当前远程来源 Manifest 幂等导入本地库，保留来源角色、版本、哈希、旧 ID 和缺口，不补写缺失资料。
2. 按 F1.4-F1.8 依次实现会议增量、Proposal 生命周期、证据、Task Packet 和 CLI/MCP。
3. 本地核心契约稳定后执行 OpenWork `OW-L0`，固定上游 SHA、许可证、网络出口和退出条件。
