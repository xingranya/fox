...
| 阶段 | 名称 | 任务 | 完成 | 进度 | 交付边界 |
|:---|:---|---:|---:|:---|:---|
| 0 | 边界、协议与黄金测试先行 | 7 | 6 | 86% | 一页边界、鸿日样本、分类标准、品牌 Agent 协议、10-20 黄金用例与 BrandBench |
...
| **合计** |  | **32** | **6** | **19%** | Phase 0-2 是当前价值验证；Phase 3 条件启动 |

## 阶段清单

- [ ] Phase 0：边界、协议与黄金测试先行（6/7）- [详情](phase-0-boundary-and-bench.md)
...
**活动任务**：F0.7：修复 BrandBench 首轮自然中文失败，完成第二轮匿名评审后重新审计技术实施就绪门
**阻塞项**：首轮 Task Packet 候选仅 12/30、自然中文 1/5，质量准入失败；远程 14GB 全量物理对账仍部分完成且存在无权限目录
**条件阶段**：Phase 3 只有 F2.8=Go 且 F3.1 团队需求门通过才启动
**规划检查点**：执行阶段已启动，当前完成 6/32
...
1. 按 `humanizer-zh` 重写普通版和 Task Packet 版，减少提纲、排比、报告腔和证据免责声明堆叠。
2. 由 Fox 完成第二轮匿名评分；自然中文至少 3/5，Task Packet 版至少三个维度超过首轮普通版。
3. 复评通过后审计 F0.7；未通过继续修复，不进入 Phase 1/OpenWork 产品实现。
