...
| 1 | 鸿日本地单用户原型 | 10 | 6 | 60% | 本地资料、SQLite 状态、会议增量、证据、Task Packet、多模型与简单界面闭环 |
| 2 | 鸿日真实工作连续验证 | 8 | 0 | 0% | 真实增量、策略/执行、多模型、匿名评审、错误修订和价值 Go/No-Go |
| 3 | 团队服务器化决策门 | 7 | 0 | 0% | 团队需求、候选架构/组件证据和独立后续 SPEC 决策 |
| **合计** |  | **32** | **13** | **41%** | Phase 0-2 是当前价值验证；Phase 3 条件启动 |

## 阶段清单

- [x] Phase 0：边界、协议与黄金测试先行（7/7）- [详情](phase-0-boundary-and-bench.md)
- [ ] Phase 1：鸿日本地单用户原型（6/10）- [详情](phase-1-hongri-local-prototype.md)
...
**活动任务**：F1.7：实现 Task Packet、品牌角色/模式装配与 Agent 运行留痕
**阻塞项**：无阶段阻塞；远程 14GB 全量物理对账和无权限目录继续作为受控缺口，不进入未登记来源
**条件阶段**：Phase 3 只有 F2.8=Go 且 F3.1 团队需求门通过才启动
**规划检查点**：Phase 0 已通过，Phase 1 已完成 F1.1-F1.6，当前完成 13/32
...
| 当前价值验证方案 | 4 阶段 / 32 项 | 13/32，执行中 | Fox/鸿日、本地单用户、黄金测试与真实工作先行 | 当前活动方案 |

旧 42 项没有已完成代码或遥测需要继承。服务器、外部组件、团队身份和高可用内容被后移为 F3.2-F3.6 的候选，只有 F3.7=Go 才重新生成实施任务。

## 下一步

1. 完成 F1.7：按任务装配当前状态、开放问题、相关证据、工作模式和品牌角色，并记录每次 Agent 运行使用的版本。
2. 完成 F1.8：通过同一 CLI/MCP 契约让 Codex、Claude 读取相同 Task Packet，不开放批准、任意 SQL 或原件硬删除。
3. 执行 OpenWork `OW-L0`，固定上游 SHA、许可证、网络出口和退出条件。
4. F1.9 再决定复用 OpenWork 壳还是使用更薄的界面，不让客户端反向成为正式状态源。
