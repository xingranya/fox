...
- 当前活动方案为 5 阶段 49 项：Phase 0 和 F1.1-F1.9 已完成；当前用 F1.10 完成鸿日业务纵切，随后建设服务器权威层、MCP/Skills/工作流接入和团队试点。
...
| 1 | 单一客户端本地纵向切片 | 10 | 9 | 90% | 本地领域核心、CLI/MCP、OpenWork 单安装包和鸿日桌面闭环 |
...
| **合计** |  | **49** | **16** | **33%** | 当前执行 F1.10，后续阶段依次过门 |

## 阶段清单

- [x] Phase 0：边界、协议与黄金测试先行（7/7）- [详情](phase-0-boundary-and-bench.md)
- [ ] Phase 1：单一客户端本地纵向切片（9/10）- [详情](phase-1-hongri-local-prototype.md)
...
**活动任务**：F1.10：接入鸿日业务纵切，通过 fraimz、黄金集和本地 E2E
**阻塞项**：鸿日业务旅程尚未接入唯一客户端；内部 macOS 包尚未签名、公证，不可向员工分发
**后续顺序**：F1.10 -> Phase 2 服务器权威基础 -> Phase 3 联网与集成 -> Phase 4 团队试点
**规划检查点**：Phase 0 已通过，Phase 1 已完成 F1.1-F1.9，当前完成 16/49
...
| 单一客户端 + 服务器权威服务 | 5 阶段 / 49 项 | 16/49，执行中 | OpenWork 唯一客户端、公司服务器权威、MCP/Skills/工作流和团队试点 | 当前活动方案 |

服务器、身份、一致性、恢复、Dify 和四个开源组件已进入正式实施顺序。外部组件仍需逐项证明收益；“拒绝并使用 NoOp”是合法结果。

## 下一步

1. 只接入已登记并获授权的鸿日来源，把当前状态、证据、Proposal、Task Packet 和 AI 工作接入唯一客户端。
2. 完成八条旅程、黄金集、本地 E2E 和 F1.10 fraimz；短时启动桌面应用后立即退出。
3. 只有 F1.10 通过，才开始 F2.1 服务器应用边界和测试基线。
