...
| 阶段 | 名称 | 任务 | 完成 | 进度 | 交付边界 |
|:---|:---|---:|---:|:---|:---|
| 0 | 边界、协议与黄金测试先行 | 7 | 5 | 71% | 一页边界、鸿日样本、分类标准、品牌 Agent 协议、10-20 黄金用例与 BrandBench |
...
| **合计** |  | **32** | **5** | **16%** | Phase 0-2 是当前价值验证；Phase 3 条件启动 |

## 阶段清单

- [ ] Phase 0：边界、协议与黄金测试先行（5/7）- [详情](phase-0-boundary-and-bench.md)
...
**活动阶段**：Phase 0：边界、协议与黄金测试先行
**活动任务**：F0.6：等待 Fox 完成 BrandBench 匿名人工评分；在不通过阶段门的前提下准备 F0.7 技术纵切、端口契约和 OpenWork 接入面
**阻塞项**：完整鸿日/V5 原件和 Fox BrandBench 人工评分仍缺失；当前只允许以 `example/` 形成演示样本和 `KNOWN_SOURCE_GAP`，不得宣称已完成真实鸿日对账或品牌质量基线
**条件阶段**：Phase 3 只有 F2.8=Go 且 F3.1 团队需求门通过才启动
**规划检查点**：执行阶段已启动，当前完成 5/32
...
1. 准备 F0.7 本地纵切、核心 Schema/Port、无网络配置与 OpenWork 接入契约，但不提前通过阶段门。
2. 生成可供领导查看的 Phase 0 脱敏进展页，明确已完成能力、测试证据和未满足门。
3. 待真实匿名候选生成后，由 Fox 完成 BrandBench 评分；评分前保持 F0.6/F0.7 未完成。
4. 为 OpenWork 准备本地纵向切片、端口和退出门，但不把 OpenWork Server 设为前置。
