...
| 1 | 鸿日本地单用户原型 | 10 | 3 | 30% | 本地资料、SQLite 状态、会议增量、证据、Task Packet、多模型与简单界面闭环 |
| 2 | 鸿日真实工作连续验证 | 8 | 0 | 0% | 真实增量、策略/执行、多模型、匿名评审、错误修订和价值 Go/No-Go |
| 3 | 团队服务器化决策门 | 7 | 0 | 0% | 团队需求、候选架构/组件证据和独立后续 SPEC 决策 |
| **合计** |  | **32** | **10** | **31%** | Phase 0-2 是当前价值验证；Phase 3 条件启动 |

## 阶段清单

- [x] Phase 0：边界、协议与黄金测试先行（7/7）- [详情](phase-0-boundary-and-bench.md)
- [ ] Phase 1：鸿日本地单用户原型（3/10）- [详情](phase-1-hongri-local-prototype.md)
...
1. 完成 F1.4：只把新会议解释成带原话位置的增量候选，并单列与当前状态的冲突。
2. 按 F1.5-F1.8 依次实现 Proposal 生命周期、证据、Task Packet 和 CLI/MCP。
