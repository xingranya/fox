...
- FoxWork 是公司定制版 OpenWork 的固定发行名，也是唯一员工客户端；界面只使用简体中文，不提供语言切换或英文回退。Brand Project OS Service 是服务器业务能力，不是第二个软件；OpenCode Runtime、Sidecar 和本机桥接仍随同一安装包分发。
...
**活动任务**：F3.2：在 FoxWork 接入 OIDC、项目选择和服务器连接
...
**需求范围权威**：新需求源文件 + `task-breakdown.md` + `milestones.md` + ADR-0004/0005/0006 + 本 MASTER
...
| 日期 | 会话 | 摘要 |
|:---|:---|:---|
| 2026-07-23 | FoxWork 发行名与中文界面约束 | Fox 将公司定制 OpenWork 的发行名固定为 FoxWork，并要求员工界面只使用简体中文，不需要语言切换或英文回退。新增 ADR-0006；F3.2 先约束联网界面，F3.13 对完整安装包做品牌和语言产物扫描。当前 49 项与执行顺序不变。 |
