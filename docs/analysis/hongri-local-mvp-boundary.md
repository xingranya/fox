...
> 本文只约束迁移前的 Phase 1，不再代表整个产品最终拓扑。F1.10 通过后按 [ADR-0005](../adr/0005-single-client-server-authority.md) 建设服务器权威服务；本文中的服务器候选标签只解释为何它不能反向成为 Phase 1 运行前置。
...
Phase 1 只有在 Fox 通过唯一客户端完成黄金旅程后才算完成，不以打包成功或测试数字代替业务验收。F1.10 通过后按批准计划进入：
