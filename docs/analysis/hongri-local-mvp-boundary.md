...
| 运行形态 | 本地优先、单用户、轻量结构化存储、简单查看与确认界面 |
| 核心目的 | 证明 AI 能正确理解长期品牌项目，减少 Fox 重复解释与状态混乱，并提升策略和语言产出质量 |
| 非目标 | 团队文件平台、通用项目管理、生产服务器、企业知识库、全量第三方平台集成 |
...
- 简单本地界面：当前状态、待确认、证据回源、模式和本轮任务；
- 10-20 个鸿日黄金用例、BrandBench 评分和 Fox 匿名人评。

## 当前不包含

以下全部标记为 `future-candidate`、`not-approved-for-current-mvp`、`review-after-hongri-pilot`：
...
CURRENT MVP 只有在 Fox 使用鸿日真实工作连续验证后才算完成，不以功能清单或测试通过代替使用价值。试点完成后单独举行 `review-after-hongri-pilot`，再决定：

- 是否需要 OpenWork 深度改造；
- 是否增加更多模型或第三方适配器；
- 是否转向团队服务器；
- 是否引入 PostgreSQL、S3、OIDC、并发和灾备；
- 是否与 FoxWork 团队文件协作线合并。
