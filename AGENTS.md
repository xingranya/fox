...
- 员工身份只按预登记的 OIDC `(issuer, subject)` 绑定；邮箱不得自动建号或合并账号。交互式人工命令必须来自有效服务器会话，撤销或绝对过期时清空令牌密文。
...
- F2.10 的 PostgreSQL 逻辑备份只用于一致快照和隔离恢复验证，不得写成生产 PITR。恢复只能进入空数据库，完成后必须核对全表摘要、事件重建投影和明确 S3 VersionId；生产 RPO/RTO、恢复点和切换仍需 Fox 确认及独立备份域验证。
